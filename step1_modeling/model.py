...
def attention(q, k, v, causal=True, use_flash_attn=True):

    # q, k, v shape: [batch_size, seq_length, num_head, head_dim]
    if use_flash_attn:
    ...
    scores = torch.matmul(q, k.transpose(-2, -1)) * scale  # [batch_size, seq_length, num_head, seq_length]

    # Apply causal mask if needed
    if causal:
        mask = torch.triu(torch.ones(scores.size(-2), scores.size(-1), dtype=torch.bool, device=scores.device), diagonal=1)
        scores.masked_fill_(mask, float('-inf'))

    # Apply softmax and multiply with values
    attn_weights = F.softmax(scores, dim=-1)
    out = torch.matmul(attn_weights, v)  # [batch_size, seq_length, num_head, head_dim]
    return out
...
class TritonRMSNorm(nn.Module):
    ...
    def forward(
        self, hidden_states, residual=None, dropout_p=0.0, prenorm=False, residual_in_fp32=False, return_dropout_mask=False, use_flash_attn=False
    ):


        if use_flash_attn:
            return layer_norm_fn(hidden_states, self.weight, self.bias, eps=self.eps)
        ...
        if dropout_p > 0.0:
            if return_dropout_mask:
                dropout_mask = torch.ones_like(hidden_states).bernoulli_(1 - dropout_p)
                hidden_states = hidden_states * dropout_mask / (1 - dropout_p)
...
class Attention(nn.Module):
    ...
    def forward(self, x, cos, sin, attention_mask=None, position_ids=None, apply_rotary_emb=False):
        ...
        out = attention(q, k, v, causal = causal, use_flash_attn=False) # [batch_size, seq_length, num_heads, head_dim]
